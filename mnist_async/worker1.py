...
if __name__ == "__main__":
    ...
    for step in range(epoch):
        ...
        for batch_cnt in range(batch_num):
            ...
            #pyfunc = tf.py_func(custom_func, worker, tf.float32)
            custom_func(worker)
            '''
            # Send worker 1's grads
            for i in range(worker.var_size):
            ...
            '''
            if batch_cnt % 10 == 0:
                print("Worker{} epoch: {} batch: {} accuracy: {} time{} ".format(rank,step,batch_cnt,worker.sess.run(worker.accuracy, feed_dict={worker.x: worker.x_test, worker.y_: worker.y_test, worker.keep_prob: 1.0}),time.time()-start))
