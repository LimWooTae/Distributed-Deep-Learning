...
class SyncWorker(Model):
    ...
    def update(self, vars):
        ...
        w_conv1 = tf.compat.v1.assign(self.w_conv1, vars[0])
        b_conv1 = tf.compat.v1.assign(self.b_conv1, vars[1])
        w_conv2 = tf.compat.v1.assign(self.w_conv2, vars[2])
        b_conv2 = tf.compat.v1.assign(self.b_conv2, vars[3])
        w_fc1 = tf.compat.v1.assign(self.w_fc1, vars[4])
...
if __name__ == "__main__":
    ...
    if rank == 0:#if self.rank == 0:
        ...
        print(p.sess.run(p.accuracy, feed_dict={p.x: p.test_x, p.y_: p.test_y_, p.keep_prob: 1.0}))
