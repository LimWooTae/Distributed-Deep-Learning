...
def deepnn(x, dropout_keep_prob=0.5):
...
class Model(object):
    def __init__(self, dataset):
        ...
        self.y_conv, self.endpoint = deepnn(self.x, dropout_keep_prob=self.keep_prob)
        ...
        # Helper values.
        self.variables = self.cross_entropy
        self.grads = self.optimizer.compute_gradients(self.cross_entropy)
        self.grads_placeholder = [(tf.placeholder("float", shape=grad[1].get_shape()), grad[1]) for grad in self.grads]
