...
if __name__ == "__main__":
    ...
    def pysend(grads,i):
        comm.Send([grads, MPI.FLOAT], dest=0, tag=i+1)
        
    def pyadd(grads,grads2):
        return grads+grads2
    ...
    phs = [tf.compat.v1.placeholder(shape=worker.var_shape[i], dtype=tf.float32) for i in range(worker.var_size)]
    ...
    for step in range(epoch):
        ...
        for batch_cnt in range(batch_num):
            ...
            for i in range(worker.var_size):
                pyoutput = tf.py_func(pysend, [worker.grads[i],i] ,[tf.float32])[0]
                #pyoutput2 = tf.py_func(pyadd, [worker.grads[i][0], worker.grads[i][0]], [tf.float32])[0]
                worker.sess.run([pyoutput])

                
            # Snd worker 1's grads
            #for i in range(worker.var_size):
